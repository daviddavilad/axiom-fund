"""Strict OOS holdout backtest — Analysis A of the holdout test design.

Runs the original 3-signal Axiom Fund strategy (GP + IVol + ResMom)
on the 2023-01 to 2024-11 window. This window has been seen during
model development; this analysis treats it as the most-out-of-sample
test available given the contamination acknowledged in
docs/holdout_test_design.md.

Strategy frozen per pre-commitment doc: 3 signals only (GP, IVol,
ResMom). PEAD is explicitly excluded by passing
signals=["gp", "ivol", "resmom"] to run_historical_backtest, because
PEAD was added to the strategy *during* the 2026-01 to 2026-06
development period and is not part of the pre-2023 frozen strategy.

This is Analysis A; Analysis B (contamination split on existing
variants) was completed in scripts/analysis/holdout_split_analysis.py.

Compare results to:
  data/cache/backtest_full_top1000/         (3-signal, 2015-2024 full)
  data/cache/holdout_split_analysis/         (Analysis B output)

Outputs to data/cache/holdout_strict_3sig/:
  - backtest_summary.parquet:   per-period DataFrame
  - weights_<date>.parquet:     per-period weights checkpoint
  - results.csv:                 summary as CSV
  - run_metadata.txt:            timestamp, parameters, environment

Estimated runtime: 50-60 minutes (cache build + 22 periods).
"""
# ruff: noqa: I001
...
RISK_AVERSION = 1.0
HOLDING_DAYS = 21
CACHE_DIR = Path("data/cache/holdout_strict_3sig")
